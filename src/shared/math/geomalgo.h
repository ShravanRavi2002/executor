...
template <class vector>
double point_on_segment_t(const vector x0,const vector x1,const vector p)
// returns nearest point on line segment x0-x1 to point p
{
  vector sx,sp,r;
  double f,l;
...
{
  //FIXME: if they're parallel, this will fail!
...
/*template <class vector>
double intersect(vector rOrigin, vector rVector, vector a, vector b)
{

  double t = ((rVector.x * rOrigin.y + rVector.y *
          (a.x - rOrigin.x)) -
          (rVector.x * b.y)) /
          (rVector.y * (a.x + b.x) -
          rVector.x * (a.y + b.y));
...
template <class vector, class real>
real ray_stretched_circle_intersect(vector rO, vector rV,vector sO,real sR, vector stretchH)
// |--- 
// |   | 
// X-sR-|   ------- 
// |    |     | stretchH 
...
// yes, it's nasty code...but needed for the new defense area!
{
  ...
  bool found=false;
  //positive circle:
  real t1=ray_sphere_intersect(rO, rV,sO+stretchH,sR);
  if (t1 >= 0.0) {
    found=true;
    t=t1;
    p=(rO+(rV*t));
  ...
  //negative circle:
  real t2=ray_sphere_intersect(rO, rV,sO-stretchH,sR);
  if (t2 >= 0.0) {
    if (found==false || t2 < t) {
      found=true;
      t=t2;
  ...
  //rotate stretchH by 90 degrees.
  //compute two planes:
  //one left, one right
  vector perp=(stretchH.perp()).norm(sR);

  real t3=ray_plane_intersect(sO+perp,perp,rO,rV);
  if (t3 >= 0.0) {
    if (found==false || t3 < t) {
      vector ptmp=(rO+(rV*t3));
      //check if intersection point is within the line-segment:
      if ((ptmp - (sO+perp)).sqlength() < stretchH.sqlength()) {
        found=true;
        t=t3;
        p=ptmp;
      }
  ...
  perp*=(-1.0);
  real t4=ray_plane_intersect(sO+perp,perp,rO,rV);
  if (t4 >= 0.0) {
    if (found==false || t4 < t) {
      vector ptmp=(rO+(rV*t4));
      //check if intersection point is within the line-segment:
      if ((ptmp - (sO+perp)).sqlength() < stretchH.sqlength()) {
        found=true;
        t=t4;
